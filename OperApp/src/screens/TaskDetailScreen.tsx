        ...
        try {
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    if (errorJson.details) errorMessage += `\n${errorJson.details}`;
                    else if (errorJson.error) errorMessage += `\n${errorJson.error}`;
                } catch (e) {
                ...
                if (response.ok) {
                    Alert.alert("Éxito", "Evidencia subida correctamente.");
                    const json = await response.json().catch(() => ({})); // Re-parse or use stored? Wait, body used already.
                    // FIX: We cannot read body twice.
                    // Refactor: Read body once.
                } else {
